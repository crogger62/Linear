...
function getLinearClient(): LinearClient {
  ...
  if (!apiKey) {
    console.error("[ERROR] Missing or empty LINEAR_API_KEY after trim (check your .env).");
    console.error("[DEBUG] Current working directory:", process.cwd());
    console.error("[DEBUG] Looking for .env at:", path.resolve(__dirname, "..", ".env"));
    // Optional: help detect invisible characters in raw (without printing the key)
    console.error("[DEBUG] Raw key length:", raw.length);
    console.error("[DEBUG] Trimmed key length:", apiKey.length);
    console.error("[DEBUG] Raw contains whitespace:", /\s/.test(raw));
...
async function main() {
  ...
  do {
    try {
      console.log(
        `[DEBUG] Fetching teams page ${page}${
          cursor ? ` (cursor: ${cursor.substring(0, 20)}...)` : " (first page)"
        }`
      );
...
main().catch((e) => {
  ...
  console.error("[DEBUG] Error type:", (e as any)?.constructor?.name || typeof e);
